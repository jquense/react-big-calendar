...
var MonthView =
  ...
  (function(_React$Component) {
    ...
    _proto.componentDidUpdate = function componentDidUpdate() {
      if (this.state.needLimitMeasure) this.measureRowLimit(this.props) //updating overlay updates are a hack fix for a bug https://jira.directi.com/browse/CWDC-80
      //TODO: Need a proper fix for this.

      var overlay = this.state.overlay

      if (overlay && overlay.events) {
        var events = overlay.events.filter(function(e) {
          return !e.isDeleted
        })

        if (events.length !== overlay.events.length) {
          this.setState({
            overlay: (0, _extends2.default)({}, overlay, {
              events: events,
            }),
          })
        }
