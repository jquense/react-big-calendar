"use strict";

var _interopRequireWildcard = require("@babel/runtime/helpers/interopRequireWildcard");

var _interopRequireDefault = require("@babel/runtime/helpers/interopRequireDefault");

exports.__esModule = true;
exports.default = void 0;

var _extends2 = _interopRequireDefault(require("@babel/runtime/helpers/extends"));

var _objectWithoutPropertiesLoose2 = _interopRequireDefault(require("@babel/runtime/helpers/objectWithoutPropertiesLoose"));

var _inheritsLoose2 = _interopRequireDefault(require("@babel/runtime/helpers/inheritsLoose"));

var _propTypes = _interopRequireDefault(require("prop-types"));

var _react = _interopRequireDefault(require("react"));

var dates = _interopRequireWildcard(require("./utils/dates"));

var _constants = require("./utils/constants");

var _TimeGridRow = _interopRequireDefault(require("./TimeGridRow"));

var Scheduler =
/*#__PURE__*/
function (_React$Component) {
  (0, _inheritsLoose2.default)(Scheduler, _React$Component);

  function Scheduler() {
    return _React$Component.apply(this, arguments) || this;
  }

  var _proto = Scheduler.prototype;

  _proto.render = function render() {
    var _this$props = this.props,
        date = _this$props.date,
        scrollToDay = _this$props.scrollToDay,
        props = (0, _objectWithoutPropertiesLoose2.default)(_this$props, ["date", "scrollToDay"]);
    var range = Scheduler.range(date, this.props);
    return _react.default.createElement(_TimeGridRow.default, (0, _extends2.default)({
      scrollToDay: scrollToDay
    }, props, {
      range: range,
      eventOffset: 15
    }));
  };

  return Scheduler;
}(_react.default.Component);

Scheduler.propTypes = process.env.NODE_ENV !== "production" ? {
  date: _propTypes.default.instanceOf(Date).isRequired
} : {};
Scheduler.defaultProps = _TimeGridRow.default.defaultProps;

Scheduler.navigate = function (date, action) {
  switch (action) {
    case _constants.navigate.PREVIOUS:
      return dates.add(date, -1, 'week');

    case _constants.navigate.NEXT:
      return dates.add(date, 1, 'week');

    default:
      return date;
  }
};

Scheduler.range = function (date, _ref) {
  var localizer = _ref.localizer;
  var firstOfWeek = localizer.startOfWeek();
  var start = dates.startOf(date, 'week', firstOfWeek);
  var end = dates.endOf(date, 'week', firstOfWeek);
  return dates.range(start, end);
};

Scheduler.title = function (date, _ref2) {
  var localizer = _ref2.localizer;

  var _Scheduler$range = Scheduler.range(date, {
    localizer: localizer
  }),
      start = _Scheduler$range[0],
      rest = _Scheduler$range.slice(1);

  return localizer.format({
    start: start,
    end: rest.pop()
  }, 'dayRangeHeaderFormat');
};

var _default = Scheduler;
exports.default = _default;
module.exports = exports["default"];