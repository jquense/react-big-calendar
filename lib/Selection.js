...
class Selection {
  constructor(node, _temp) {
    var { global = false, longPressThreshold = 250 } =
      _temp === void 0 ? {} : _temp
  ...
  _addLongPressListener(handler, initialEvent) {
    ...
    var handleTouchStart = initialEvent => {
      timer = setTimeout(() => {
        cleanup()
  ...
  _addInitialEventListener() {
    var removeMouseDownListener = addEventListener('mousedown', e => {
    ...
    var removeTouchStartListener = addEventListener('touchstart', e => {
      this._removeInitialEventListener()
  ...
  _dropFromOutsideListener(e) {
    var { pageX, pageY, clientX, clientY } = getEventCoordinates(e)
  ...
  _handleInitialEvent(e) {
    ...
    var { clientX, clientY, pageX, pageY } = getEventCoordinates(e)
    ...
    if (!this.globalMouse && node && !(0, _contains.default)(node, e.target)) {
      var { top, left, bottom, right } = normalizeDistance(0)
  ...
  _handleTerminatingEvent(e) {
    var { pageX, pageY } = getEventCoordinates(e)
  ...
  isClick(pageX, pageY) {
    var { x, y, isTouch } = this._initialEventData
    return (
      !isTouch &&
