...
function eventLevels(_rowSegments, limit) {
  if (limit === void 0) {
    limit = Infinity
  }
  ...
  var rowSegments = _rowSegments.concat() //If a segment has a key positionTop = true, handle it first.

  for (i = 0; i < rowSegments.length; i++) {
    var _seg = rowSegments[i] //Allow only one for now, we'll think about supporting multiple top events

    if (_seg.event.positionTop) {
      levels[0] = [_seg]
