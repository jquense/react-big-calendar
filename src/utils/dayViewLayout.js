 ...
 */
let getSiblings = (idx, { events, startAccessor, endAccessor, min, totalMin, step, timeslots }) => {
  let nextIdx = idx;
  let siblings = [];
...
let getChildGroups = (
  ...
  while (
    isChild(idx, nextIdx, { events, startAccessor, endAccessor, min, totalMin, step, timeslots })
  ) {
...
export default function getStyledEvents({
  ...
  while (idx < events.length) {
    ...
    idx += 1 + siblings.length + childGroups.reduce((total, group) => total + group.length, 0);
  }
